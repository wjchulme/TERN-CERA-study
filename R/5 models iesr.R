...
data_long123 <- read_rds(file=here::here("processed-data", "data_long123.rds")) %>% 
  filter(survey !=1) %>%
  mutate(
    iesrsum0123_jit = iesrsum0123 + runif(n(), -0.2,0.2),
  ) %>% droplevels()
data_long123_ghq <- read_rds(file=here::here("processed-data", "data_long123_ghq.rds")) %>% filter(survey !=1)
data_long123_iesr <- read_rds(file=here::here("processed-data", "data_long123_iesr.rds")) %>% filter(survey !=1)
...
# choose variables
# can only use those that were available across surveys 2+3, or those that are time-invariant
...
univariable_div <- c(
  ...
  "supportcol", "supportfam",
  NULL
)

univariable_cat <- c(
  "gender", "ethnicity", "ethnicity3", "seniority","comb_dept2", "region", 
  "comb_outbreak_any",
...
models <- 
  ...
  mutate(
    ...
    mod.lm_s1 = map2(dat, variables, ~lmerTest::lmer(data=.x, formula=as.formula(paste0("iesrsum0123 ~ survey_chr +", .y, "+ (1| ptid)")))),
    mod.lm_s2 = map2(dat, variables, ~lmerTest::lmer(data=.x, formula=as.formula(paste0("iesrsum0123 ~ survey_chr *", .y, "+ (1| ptid)")))),
...
#models <- read_rds(file=here::here("figures", "survey-123", "IESR", "models.rds"))
#models %>% select(name, n, n_missing, pct_missing, starts_with("pval"), starts_with("R2")) %>% View()
...
models_plots <- models %>%
  transmute(
    ...
    plotraw_hist = pmap(lst(catvar=variables, contvar="iesrsum0123", data=list(data_descr), xlab = "IES-R score"), plotrawdata_hist),
    plotraw_jit = pmap(lst(catvar=variables, contvar="iesrsum0123", data=list(data_descr), xlab = "IES-R score", breaks = list(c(0, 10, 20, 30, 40, 50, 60, 70, 80)), labels = list(c("0", "10", "20", "30", "40", "50", "60", "70", "80"))), plotrawdata_jit),
    ploteffect_1 = pmap(lst(effects=effects_s1, limits = list(c(5, 25)), colourtype = colourtype, xlab="Average IES-R score", title=name), ploteffect),
    ploteffect_2 = pmap(lst(effects=effects_s2, limits = list(c(5, 25)), colourtype = colourtype, xlab="Average IES-R score", title=name), ploteffect),
...
write_rds(models_plots, file=here::here("outputs", "IESR", "models_plots.rds"), compress="gz")
...
models %>%
  ...
  pwalk(
    function(variables, summary_table){
      write_csv(summary_table, file = here::here("outputs", "IESR", "descriptive", "tables", paste0("summary_",variables, ".csv")))
    }
  )
...
models_plots %>%
  ...
  pwalk(
    function(variables,effects){
      write_csv(effects, file = here::here("outputs", "IESR", "models", "tables", paste0("model2_",variables, ".csv")))
    }
  )
...
plot_varexplained <- models %>%
  ...
  labs(
    title="Variation in IES-R explained by each model",

...
#raw_dataUK <- readr::read_csv(here::here("redcap exports","UK", "COVID19EmergencyResp_DATA_2020-05-06_2034.csv"))
#raw_dataUK <- readr::read_csv(here::here("redcap exports","UK", "COVID19EmergencyResp_DATA_2020-06-09_2049.csv"))
raw_dataUK <- readr::read_csv(here::here("redcap exports","UK", "COVID19EmergencyResp_DATA_2020-06-30_2117.csv"))

#raw_dataUK_labels <- readr::read_csv(here::here("redcap exports", "UK", "COVID19EmergencyResp_DATA_LABELS_2020-05-06_2034.csv"))
#raw_dataUK_labels <- readr::read_csv(here::here("redcap exports", "UK", "COVID19EmergencyResp_DATA_LABELS_2020-06-09_2049.csv"))
raw_dataUK_labels <- readr::read_csv(here::here("redcap exports", "UK", "COVID19EmergencyResp_DATA_LABELS_2020-06-30_2117.csv"))
...
#raw_dataRI <- readr::read_csv(here::here("redcap exports","Ireland","COVID19EmergencyResp_DATA_2020-05-24_2127.csv"))
#raw_dataRI <- readr::read_csv(here::here("redcap exports","Ireland","COVID19EmergencyResp_DATA_2020-06-09_2305.csv"))
raw_dataRI <- readr::read_csv(here::here("redcap exports","Ireland","COVID19EmergencyResp_DATA_2020-06-30_2158.csv"))

#raw_dataRI_labels <- readr::read_csv(here::here("redcap exports","Ireland","COVID19EmergencyResp_DATA_LABELS_2020-05-24_2127.csv"))
#raw_dataRI_labels <- readr::read_csv(here::here("redcap exports","Ireland","COVID19EmergencyResp_DATA_LABELS_2020-06-09_2305.csv"))
raw_dataRI_labels <- readr::read_csv(here::here("redcap exports","Ireland","COVID19EmergencyResp_DATA_LABELS_2020-06-30_2158.csv"))
...
raw_dataRI2 <- readr::read_csv(here::here("redcap exports","Ireland","COVID19EmergencyResp_DATA_2020-06-30_2116 survey2.csv"))
raw_dataRI2_labels <- readr::read_csv(here::here("redcap exports","Ireland","COVID19EmergencyResp_DATA_LABELS_2020-06-30_2116 survey2.csv"))
...
factor_dct <- dictionary %>%
  filter(!is.na(level) | str_starts(R, "feelings_")) %>%
  mutate(
    short=NA,
  ...
  add_row(
    ...
    plottype="stack",
    code=list(1:7),
    level=list(c("Feelings that you made a contribution", "A sense of personal accomplishment", "Improved confidence and self esteem", 
                 "Increased compassion", "Re-evaluation of self and purpose", "Work satisfaction", "A sense of team cohesion")),
    short=list(c("contribution", "accomplish", "confidence", "compassion", "purpose", "satisfaction", "cohesion"))
  ) %>%
  add_row(
    R="selfisolatereason",
    ...
    vartype="personal",
    code=list(1:6),
    level=list(c("Personal symptoms","Personal diagnosis of COVID-19","Symptoms of a member of the household","Exposure to a positive case of COVID-19 in the work environment","Exposure to a positive case of COVID-19 in your personal environment","Other")),
    short=list(c("symptomspersonal", "diagnosispersonal", "symptomshousehold", "diagnosiswork", "diagnosisplay", "other"))
  ) %>%
  add_row(
    R="redeployed_where2_s1",
    name="Redeployment",
...
dct_hosp <- dictionary %>% 
  ...
  left_join((readxl::read_xlsx(here::here("freetext mapping", "remapped hospitals.xlsx"))), by = "hosp_code") %>%
  select(-hosp) %>%
  filter(!is.na(new_hosp_code))

corrections_ft_hosp <- read_csv(here::here("freetext mapping", "corrected ft hospitals.csv")) %>%
  left_join(dct_hosp %>% select(hosp_code, hosp_level), by="hosp_code")# %>%
  #janitor::get_dupes(ft_hosp)

corrections_ft_grade <- read_csv(here::here("freetext mapping", "corrected ft grades.csv")) %>%
  mutate(
...
corrections_hosp <- data0 %>%
  ...
  mutate(
    region = factor(region, levels=c("East Midlands", "East of England", "London", "North East", "North West", "South East", "South West", "West Midlands", "Yorkshire and the Humber",
                                     "Northern Ireland", "Scotland", "Wales", "Dublin", "Rest of Ireland")),
    ...
    nation = case_when(
      ...
      region %in% c("East Midlands", "East of England", "London", "North East", "North West", "South East", "South West", "West Midlands", "Yorkshire and the Humber") ~ "England",
...
mutate(
  ...
  seniority = case_when(
    grade %in% c("F1", "F2", "GP Trainee","ST1","ST2", "ST3", "Clinical Fellow (F2-ST3 Level)", 
                 "Intern", "Senior House Officer (Training scheme)", "Senior House Officer (Standalone)") ~ "Junior Doctor",
    grade %in% c("ST4", "ST5", "ST6", "ST7", "ST8", "Clinical Fellow (>=ST4 Level)", 
                 "Registrar (Training scheme)", "Registrar (Standalone)", "Specialist Registrar", "Senior Registrar") ~ "Middle Grade Doctor",
    grade %in% c("Associate Specialist", "Staff Grade", "GP", "CESR Doctor") ~ "Other Senior Doctor",
    grade %in% c("Consultant") ~ "Senior Doctor (Consultant Grade)",
    TRUE ~ "other"
  ) %>% factor(levels=c("Junior Doctor","Middle Grade Doctor","Other Senior Doctor", "Senior Doctor (Consultant Grade)")),
  ...
  selfisolatereason_NA_s1 = if_else((selfisolate_s1=="Yes"),
  ...
  redeployed_where2_s1 = 
    ...
    factor(levels = c("Not redeployed", factor_dct$level[factor_dct$R=="redeployed_where_s1"][[1]])),
  ...
  redeployed_where2_s2 = 
    ...
    factor(levels = c("Not redeployed", factor_dct$level[factor_dct$R=="redeployed_where_s2"][[1]])),
  ...
  feelings_NA_s2 = ((!(feelings_contribution_s2))+(!(feelings_accomplish_s2))+(!(feelings_purpose_s2))+(!(feelings_confidence_s2))+(!(feelings_compassion_s2))+(!(feelings_satisfaction_s2))+(!(feelings_cohesion_s2))==0),
  feelings_contribution_s2 = if_else(feelings_NA_s2, NA_character_, if_else(feelings_contribution_s2, "Yes", "No" )) %>% factor(levels=c("No", "Yes")),
  feelings_accomplish_s2 = if_else(feelings_NA_s2, NA_character_, if_else(feelings_accomplish_s2, "Yes", "No" )) %>% factor(levels=c("No", "Yes")),
  feelings_confidence_s2 = if_else(feelings_NA_s2, NA_character_, if_else(feelings_confidence_s2, "Yes", "No" )) %>% factor(levels=c("No", "Yes")),
  feelings_compassion_s2 = if_else(feelings_NA_s2, NA_character_, if_else(feelings_compassion_s2, "Yes", "No" )) %>% factor(levels=c("No", "Yes")),
  feelings_purpose_s2 = if_else(feelings_NA_s2, NA_character_, if_else(feelings_purpose_s2, "Yes", "No" )) %>% factor(levels=c("No", "Yes")),
  feelings_satisfaction_s2 = if_else(feelings_NA_s2, NA_character_, if_else(feelings_satisfaction_s2, "Yes", "No" )) %>% factor(levels=c("No", "Yes")),
  feelings_cohesion_s2 = if_else(feelings_NA_s2, NA_character_, if_else(feelings_cohesion_s2, "Yes", "No" )) %>% factor(levels=c("No", "Yes")),
  ...
  feelings_NA_s3 = ((!(feelings_contribution_s3))+(!(feelings_accomplish_s3))+(!(feelings_purpose_s3))+(!(feelings_confidence_s3))+(!(feelings_compassion_s3))+(!(feelings_satisfaction_s3))+(!(feelings_cohesion_s3))==0),
  feelings_contribution_s3 = if_else(feelings_NA_s3, NA_character_, if_else(feelings_contribution_s3, "Yes", "No" )) %>% factor(levels=c("No", "Yes")),
  feelings_accomplish_s3 = if_else(feelings_NA_s3, NA_character_, if_else(feelings_accomplish_s3, "Yes", "No" )) %>% factor(levels=c("No", "Yes")),
  feelings_confidence_s3 = if_else(feelings_NA_s3, NA_character_, if_else(feelings_confidence_s3, "Yes", "No" )) %>% factor(levels=c("No", "Yes")),
  feelings_compassion_s3 = if_else(feelings_NA_s3, NA_character_, if_else(feelings_compassion_s3, "Yes", "No" )) %>% factor(levels=c("No", "Yes")),
  feelings_purpose_s3 = if_else(feelings_NA_s3, NA_character_, if_else(feelings_purpose_s3, "Yes", "No" )) %>% factor(levels=c("No", "Yes")),
  feelings_satisfaction_s3 = if_else(feelings_NA_s3, NA_character_, if_else(feelings_satisfaction_s3, "Yes", "No" )) %>% factor(levels=c("No", "Yes")),
  feelings_cohesion_s3 = if_else(feelings_NA_s3, NA_character_, if_else(feelings_cohesion_s3, "Yes", "No" )) %>% factor(levels=c("No", "Yes")),
...
write_rds(x=data2, path=here::here("processed data", "data_all.rds"))
